...
- (void)showTabBar:(CDVInvokedUrlCommand*)command
{
    ...
    if ([command.arguments objectAtIndex:0])
	{
        ...
        atBottom = [[[command.arguments objectAtIndex:0] objectForKey:@"position"] isEqualToString:@"bottom"];
    }
	if(height == 0)
	{
		height = 49.0f;
...
- (void) createTabBarItem:(CDVInvokedUrlCommand*)command
{
    ...
    if ([imageName length] > 0) {
        ...
        if ([imageName isEqualToString:@"tabButton:More"])       systemItem = (UIBarButtonSystemItem) UITabBarSystemItemMore;
        if ([imageName isEqualToString:@"tabButton:Favorites"])  systemItem = (UIBarButtonSystemItem) UITabBarSystemItemFavorites;
        if ([imageName isEqualToString:@"tabButton:Featured"])   systemItem = (UIBarButtonSystemItem) UITabBarSystemItemFeatured;
        if ([imageName isEqualToString:@"tabButton:TopRated"])   systemItem = (UIBarButtonSystemItem) UITabBarSystemItemTopRated;
        if ([imageName isEqualToString:@"tabButton:Recents"])    systemItem = (UIBarButtonSystemItem) UITabBarSystemItemRecents;
        if ([imageName isEqualToString:@"tabButton:Contacts"])   systemItem = (UIBarButtonSystemItem) UITabBarSystemItemContacts;
        if ([imageName isEqualToString:@"tabButton:History"])    systemItem = (UIBarButtonSystemItem) UITabBarSystemItemHistory;
        if ([imageName isEqualToString:@"tabButton:Bookmarks"])  systemItem = (UIBarButtonSystemItem) UITabBarSystemItemBookmarks;
        if ([imageName isEqualToString:@"tabButton:Search"])     systemItem = (UIBarButtonSystemItem) UITabBarSystemItemSearch;
        if ([imageName isEqualToString:@"tabButton:Downloads"])  systemItem = (UIBarButtonSystemItem) UITabBarSystemItemDownloads;
        if ([imageName isEqualToString:@"tabButton:MostRecent"]) systemItem = (UIBarButtonSystemItem) UITabBarSystemItemMostRecent;
        if ([imageName isEqualToString:@"tabButton:MostViewed"]) systemItem = (UIBarButtonSystemItem) UITabBarSystemItemMostViewed;
        if (systemItem != -1)
            item = [[UITabBarItem alloc] initWithTabBarSystemItem: (UITabBarSystemItem)systemItem tag:tag];
    }
    ...
    if ([[command.arguments objectAtIndex:4] objectForKey:@"badge"])
        item.badgeValue = [[command.arguments objectAtIndex:4] objectForKey:@"badge"];
...
- (void)updateTabBarItem:(CDVInvokedUrlCommand*)command
{
    ...
    if (item)
        item.badgeValue = [[command.arguments objectAtIndex:1] objectForKey:@"badge"];
}
...
- (void)createToolBar:(CDVInvokedUrlCommand*)command
{
    ...
    NSDictionary* toolBarSettings = [command.arguments objectAtIndex:0];// options;//[settings objectForKey:@"ToolBarSettings"];
    if (toolBarSettings) 
	{
...
- (void)createActionSheet:(CDVInvokedUrlCommand*)command
{
	...
	NSString* title = [[command.arguments objectAtIndex:0] objectForKey:@"title"];
	...
	int count = [[command.arguments objectAtIndex:1] count];
	for(int n = 0; n < count; n++)
	{
		[ actionSheet addButtonWithTitle:[[command.arguments objectAtIndex:1] objectAtIndex:n]];
	}
	
	if([[command.arguments objectAtIndex:0] objectForKey:@"cancelButtonIndex"])
	{
		actionSheet.cancelButtonIndex = [[[command.arguments objectAtIndex:0] objectForKey:@"cancelButtonIndex"] intValue];
	}
	if([[command.arguments objectAtIndex:0] objectForKey:@"destructiveButtonIndex"])
	{
		actionSheet.destructiveButtonIndex = [[[command.arguments objectAtIndex:0] objectForKey:@"destructiveButtonIndex"] intValue];
	}
